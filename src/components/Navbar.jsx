...
const Navbar = () => {
    ...
    return (
        <div className="flex items-center w-full justify-between pt-1.5 px-1 md:px-4">
            ...
            <button className="flex  justify-evenly rounded-md hover:bg-slate-400 p-1" onClick={handleOpen}>
                <img className="w-4 h-4  ml-1" src={"darkModeWhite.svg"} alt="" />
                <img className="w-4 h-4  ml-1" src={"wifiWhite.svg"} alt="" />
                <img className="w-6 ml-2" src="sound.svg" alt="" />
                <img className="w-4 ml-2" src="shutdownWhite.svg" alt="" />
            </button>
            {open ? (
                <ul ref={dropdownRef} className={`absolute w-48 bg-white p-2 top-10 right-2 flex flex-col items-center  text-center rounded-md dark:bg-[#242424] z-50`}>
                    <button className="w-full flex flex-row-reverse items-center justify-end hover:bg-blue-700 rounded" onClick={() => handleDropDwn("wifi")}>
                        <span className="text-back dark:text-white text-center  ml-1">{wifi ? "Pranjal" : "connect wifi"}</span>
                        <img className="w-4 h-4  ml-1" src={wifi ? (theme ? "wifiWhite.svg" : "wifi.svg") : (theme ? "wifiDisconnect.svg" : "wifiDisconnectDark.svg")} alt="" />
                    </button>
                    <button id="theme-toggle" className="w-full flex flex-row-reverse items-center justify-end hover:bg-blue-700 rounded" onClick={() => handleDropDwn("dark")}>
                        <span className="text-back dark:text-white text-center  ml-1">{theme ? "Dark Mode " : "Light Mode "} on</span>
                        <img className="w-4 h-4  ml-1" src={theme ? "darkModeWhite.svg" : "lightMode.svg"} alt="" />
                    </button>
                    <button className="w-full flex flex-row-reverse items-center justify-end hover:bg-blue-700 rounded" onClick={() => handleDropDwn("shutdown")}>
                        <span className="text-back dark:text-white  text-center  ml-1">shutdown</span>
                        <img className="w-4 h-4  ml-1" src={theme ? "shutdownWhite.svg" : "shutdownDark.svg"} alt="" />
                    </button>
                </ul>
            ) : null}
